...
// Navbar Clock - Real Time with AM/PM for Halifax, NS (Atlantic Time)
function updateNavbarClock() {
  const now = new Date();

  // Get time in Halifax, Nova Scotia (Atlantic Time)
  // Atlantic Standard Time (AST) is GMT-4 (winter)
  // Atlantic Daylight Time (ADT) is GMT-3 (summer)
  // Use toLocaleString with Halifax timezone
  const halifaxTime = new Date(now.toLocaleString("en-US", {
    timeZone: "America/Halifax"
  }));

  let hours = halifaxTime.getHours();
...
const roles = ["Full-Stack Web Developer", "Ai Integration"];
...
// Crypto Prices Fetcher (BTC, ETH, LTC)
async function fetchCryptoPrices() {
  const btcValueElement = document.querySelector(".btc-value");
  const ethValueElement = document.querySelector(".eth-value");
  const ltcValueElement = document.querySelector(".ltc-value");

  console.log("Fetching crypto prices...", { btcValueElement, ethValueElement, ltcValueElement });
  ...
  try {
    ...
    // Format BTC price
    if (data.bitcoin && btcValueElement) {
      ...
      btcValueElement.textContent = formattedBTC;
      console.log("BTC price updated:", formattedBTC);
    }

    // Format ETH price
    if (data.ethereum && ethValueElement) {
      const ethPrice = data.ethereum.usd;
      const formattedETH = ethPrice.toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
        minimumFractionDigits: 0,
        maximumFractionDigits: 0,
      });
      ethValueElement.textContent = formattedETH;
      console.log("ETH price updated:", formattedETH);
    }

    // Format LTC price
    if (data.litecoin && ltcValueElement) {
      const ltcPrice = data.litecoin.usd;
      const formattedLTC = ltcPrice.toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
        minimumFractionDigits: 0,
        maximumFractionDigits: 0,
      });
      ltcValueElement.textContent = formattedLTC;
      console.log("LTC price updated:", formattedLTC);
    }
  } catch (error) {
    console.error("Crypto price fetch error:", error);
    if (btcValueElement) btcValueElement.textContent = "N/A";
    if (ethValueElement) ethValueElement.textContent = "N/A";
    if (ltcValueElement) ltcValueElement.textContent = "N/A";
  }
}

// Initialize crypto prices after DOM is ready
...
// RESET APPRECIATE COUNTER - Run this once to reset to 0
// Uncomment the line below and refresh page once, then comment it back
import { set } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-database.js";
//set(countRef, 0);  // Uncomment this line to reset, then comment back
